...
전공 영어 + 토익 + JLPT 단어장을 분리하여 관리하고,  
보다 효율적으로 학습해 보자는 목적입니다.
...
- `src/vocab/MainFrame.java` : GUI 메인 화면 (단어장 선택 기능 포함)
...
| **BookType**   | 완성 | 전공/토익/JLPT 단어장을 구분하는 enum 정의 및 파일 경로 관리. |
| **FileManager**| 완성 | 단어장 선택에 따른 파일 저장/불러오기 구현. UTF-8 기반 처리 및 디렉토리 자동 생성 지원. |
| **QuizManager**| 완성 | 랜덤 문제 출제·정답 판별 구현. 문제 수·정답 수·정답률 통계 기능 포함. |
| **App**        | 완성 | 프로그램 실행 구조 정리됨. 선택된 단어장을 기반으로 메인 화면 실행. |
| **MainFrame**  | 완성 | 단어 추가/삭제/목록/저장/불러오기/퀴즈/통계 기능 완성. 단어장 선택 기능 추가. |
| **QuizFrame**  | 완성 | 퀴즈 GUI 완성. 정답 확인 및 다음 문제 자동 출제. 퀴즈 기록 정상 반영. |
| **StatsFrame** | 완성 | 단어장·퀴즈 통계 출력 구현. 정답률·전체 단어 수 등 정보 제공. |
